...
$$\min_{\beta \in \mathbb{R}^p} \frac{1}{2n} (y - X\beta)^T (y - X\beta) + \lambda \left[ \frac{1}{2} (1 - \alpha) \beta^T \beta + \alpha ||\beta||_1 \right]$$

Given covariate matrix $X$ and response vector $y$, the function `yl2883::elnet_coord` solves exactly the problem:
```{r}
library(mvtnorm)
p=20
n=100
